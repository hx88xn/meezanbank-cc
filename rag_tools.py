...
PINECONE_NAMESPACE = os.getenv("PINECONE_NAMESPACE", "meezanbank-data")
...
def _sync_embed_and_query(query: str, top_k: int):
    ...
    print(f"⏱️ [OPENAI RAG] embed: {embed_ms:.0f}ms {'(cache hit)' if cache_hit else '(api call)'} | pinecone: {query_ms:.0f}ms | total: {embed_ms + query_ms:.0f}ms")
    return results
